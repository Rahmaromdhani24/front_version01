...
public class SelectionInitialeController{
	 @FXML
    ...
    @FXML
    private ComboBox<String> ListespecificationsMesure;
    ...
    public void initialize() {
        ...
        if (TorsadageInformations.specificationsMesure != null) {
        	ListespecificationsMesure.setValue((TorsadageInformations.specificationsMesure));
        }

        if (TorsadageInformations.codeControleSelectionner != null) {
            listeCodeControle.setValue(TorsadageInformations.codeControleSelectionner);
        }

        if (TorsadageInformations.projetSelectionner != null) {
            listeProjets.setValue(TorsadageInformations.projetSelectionner);
        }
    }
    ...
    void submit(ActionEvent event) {
        if (listeCodeControle.getValue() == null ||
            ListespecificationsMesure.getValue() == null ||
            ...
            try {       
                FXMLLoader loader = new FXMLLoader(getClass().getResource("/Front_java/Torsadage/RemplirTorsadage.fxml"));
                Parent dashboard2Root = loader.load();
                dashboard2Root.getStylesheets().add(getClass().getResource("/Front_java/Torsadage/RemplirTorsadage.css").toExternalForm());
    ...
    private List<String> getSpecificationsMesuresFromApi() throws Exception {
        // Récupérer le token
        String token = AppInformations.token;

        // Créer une requête HTTP avec l'en-tête Authorization
        HttpRequest request = HttpRequest.newBuilder()
                .uri(URI.create("http://localhost:8281/operations/torsadage/specificationsMesure"))
        ...
        if (response.statusCode() == 200) {
            ...
            List<String> specificationsMesure = objectMapper.readValue(response.body(), List.class);

            return specificationsMesure;
        } else {
    ...
    private void populateComboBoxSections() {
        Task<List<String>> task = new Task<>() {
            ...
            protected List<String> call() throws Exception {
                return getSpecificationsMesuresFromApi();
            }
        };
        ...
        task.setOnSucceeded(event -> {
            ...
            ListespecificationsMesure.setItems(observableList);
            ListespecificationsMesure.getSelectionModel().selectedItemProperty().addListener((obs, oldValue, newValue) -> {
                if (newValue != null) {
                    TorsadageInformations.specificationsMesure = newValue;
                }
            });
        });
    ...
    private void loadCodesControles() {
        ...
        task.setOnSucceeded(event -> {
            ...
            listeCodeControle.getSelectionModel().selectedItemProperty().addListener((obs, oldValue, newValue) -> {
                if (newValue != null) {
                    TorsadageInformations.codeControleSelectionner = newValue;
    ...
    private void loadProjets() {
        ...
        task.setOnSucceeded(event -> {
            ...
            listeProjets.getSelectionModel().selectedItemProperty().addListener((obs, oldValue, newValue) -> {
                if (newValue != null) {
                    TorsadageInformations.projetSelectionner = newValue;
                }
            });
        });
        ...
        Task<Void> task = new Task<>() {
            ...
            protected Void call() throws Exception {
                ...
                if (response.statusCode() == 200) {
                    ...
                    Platform.runLater(() -> {
                        List<Label> descriptionsLabels = List.of(description1, description2, description3);
