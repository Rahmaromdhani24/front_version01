...
public class ChartSertissageIDCTraction extends Application {
    ...
    public void start(Stage primaryStage) {
        ...
        // Titre principal
        Label title = new Label("Graphique de Sertissage IDC - Traction");
        ...
        Label legend1 = createLegend("Côté 1", Color.RED);
        Label legend2 = createLegend("Côté 2", Color.BLUE);

        legendBox.getChildren().addAll(legend1, legend2);

        StackPane chart = createMoyenneChartWithZones(
                45, 48, 51, 55, // Côté 1
                42, 52, 57, 60 // Côté 2
        );

        root.getChildren().addAll(title, legendBox, chart);

        Scene scene = new Scene(root, 650, 500);
        primaryStage.setScene(scene);
        primaryStage.setTitle("Zones Colorées - Traction");
        primaryStage.show();
    }
    ...
    public static StackPane createMoyenneChartWithZones(
            double y1a, double y1b, double y1c, double y1d,  // Côté 1
            double y2a, double y2b, double y2c, double y2d  // Côté 2
    ) {
        CategoryAxis xAxis = new CategoryAxis();
        NumberAxis yAxis = new NumberAxis(40, 110, 5);
        ...
        serie2.setName("Côté 2"); // Légende pour Côté 2
        serie2.getData().add(new XYChart.Data<>("Éch1 (C2)", y2a));  // Éch1 pour Côté 2
        serie2.getData().add(new XYChart.Data<>("Éch2 (C2)", y2b));  // Éch2 pour Côté 2
        serie2.getData().add(new XYChart.Data<>("Éch3 (C2)", y2c));  // Éch3 pour Côté 2
        serie2.getData().add(new XYChart.Data<>("ÉchFin (C2)", y2d));  // ÉchFin pour Côté 2

        // Ajouter les séries au graphique
        chart.getData().addAll(serie1, serie2);
